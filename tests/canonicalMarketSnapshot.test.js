...
import { buildCanonicalMarketSnapshot, createCanonicalStudyFingerprint } from '../backend/src/canonicalMarketSnapshot.js';
...
function readySnapshot(overrides = {}) {
  ...
  return {
    ...
    authoritativeFreshness: {
      authorityGate: 'PASS',
      freshnessGate: 'PASS',
      timestampAuthority: 'INDEPENDENT_PROVIDER_EVENT_TIME',
...
test('builds a ready canonical snapshot only with explicit admission, temporal authority and closed candles', () => {
  ...
  assert.equal(value.closedCandles.length, 50);
  assert.equal(value.freshnessContractMs, 30_000);
  assert.equal(value.admissionProof.state, 'ADMITTED');
});
...
test('fails closed without admission proof or approved temporal authority', () => {
  ...
  const badTemporal = buildCanonicalMarketSnapshot({
    snapshot: readySnapshot({ authoritativeFreshness: { authorityGate: 'FAIL', freshnessGate: 'UNVERIFIED', timestampAuthority: 'UNRESOLVED', freshnessContractMs: 30_000 } }),
    admission,
    now: NOW
  });
  assert.ok(badTemporal.reasons.includes('CANONICAL_TEMPORAL_AUTHORITY_NOT_APPROVED'));
  assert.equal(badTemporal.valid, false);
});
...
test('fails closed when closed-candle proof is missing or current/open bar leaks into structural input', () => {
  ...
  assert.equal(value.closedCandles.length, 50);
  assert.equal(value.closedCandles.at(-1).timestamp, new Date(Date.parse(base.latestClosedCandleTimestamp)).toISOString());
});
...
test('study fingerprint is deterministic and ignores live price changes while closed structure is unchanged', () => {
  ...
  const fa = createCanonicalStudyFingerprint(a);
  const fb = createCanonicalStudyFingerprint(b);
  assert.equal(fa.eligible, true);
  assert.equal(fa.hash, fb.hash);
});
