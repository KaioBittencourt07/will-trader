...
export function resolveProspectiveOutcome(record, { price, timestamp = new Date().toISOString(), valid = true, status = 'OK' } = {}, now = Date.now()) {
  ...
  if (!Number.isFinite(referenceMs)) return { resolved: true, outcome: 'DATA_INVALID', reason: 'INVALID_REFERENCE_TIMESTAMP', dueAt: new Date(dueAt).toISOString() };
  ...
  if (valid === false || status === 'STALE' || status === 'INVALID' || !Number.isFinite(entry) || !Number.isFinite(exit)) {
