...
export const MAX_MARKET_CLOCK_LEAD_MS = 1_000;
...
export function buildExecutionTiming({ signalTime, marketTime, executionDelayMs = 0, windowBeforeMs = 2_000, windowAfterMs = 3_000, entryWindowStartMs = null, entryWindowEndMs = null, expirySeconds = 60 } = {}) {
  ...
  const marketLeadMs = Number.isFinite(signalMs) && Number.isFinite(marketMs) ? marketMs - signalMs : null;
  if (!Number.isFinite(signalMs) || !Number.isFinite(marketMs) || marketLeadMs > MAX_MARKET_CLOCK_LEAD_MS) {
    return { status: TIMING_STATUS.INVALID, valid: false, reason: 'INVALID_TIMING', marketLeadMs };
  }
  ...
  if (validUntilMs < validFromMs || clickMs < validFromMs || clickMs > validUntilMs) return { status: TIMING_STATUS.INVALID, valid: false, reason: 'INVALID_ENTRY_WINDOW', marketLeadMs };
  return { valid: true, status: TIMING_STATUS.TOO_EARLY, marketTime, signalTime, marketLeadMs, clickTime: new Date(clickMs).toISOString(), validFrom: new Date(validFromMs).toISOString(), validUntil: new Date(validUntilMs).toISOString(), expiryTime: new Date(clickMs + expirySeconds * 1000).toISOString() };
}
