 ...
 * most evidenced released plan among the ones the deterministic engine already
 * approved for a bounded future/manual entry window.
 */
...
export function selectBestOpportunity(analyses = []) {
  const released = analyses.filter((item) => item?.decision?.releaseEligible === true && !item.decision.blocked && ['BUY', 'SELL'].includes(item.decision.direction));
  if (!released.length) return { recommendation: null, reason: 'Nenhum ativo atingiu os critérios de liberação de plano. WAIT é a decisão correta.' };
  const ranked = released
  ...
  return {
    ...
    reason: 'Ativo com maior qualidade de evidência entre os planos já liberados para revisão manual.'
  };
}
