...
recover takes a supplied complete history snapshot and returns a new records array, materialized IDs and sealedCycleIds derived ONLY from seal commits. These IDs prove inventory seal, NOT settlement or OOS completeness. It never writes history or scheduler state. Existing matching records are preserved including later settlements; duplicate IDs or changed membership block recovery. Repeating recovery with its returned history does not duplicate creation. A future integration must apply missing insertions transactionally and restore scheduler termination from confirmed seal evidence only.

An existing zero-length journal.jsonl alongside any manifest projection is quarantined as JOURNAL_INVALID, never treated as a fresh journal or a generation reset. Removal of complete trailing WAL events together with loss/rollback of their projections may be indistinguishable from an earlier historical state without an external monotonic authority. This foundation does not claim arbitrary rollback or power-loss protection.
...
verifyManifestAgainstHistory checks full cycle membership independent of statistical eligibility: exact IDs, duplicates, generation, digest, SEALED state and terminal settlement timestamps. Resolution requires status CLOSED, a valid settledAt and an explicit terminal label WIN, LOSS, TIE or DATA_INVALID. Missing/unknown labels or invalid status/timestamps yield PENDING_RECORD and complete=false, without performance calculation. Pending records block completeness without changing inventory. Monitor completion remains an additional future runtime requirement.
