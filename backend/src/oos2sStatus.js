...
export function inspectOos2sStatus({evidenceDirectory,history=[],freezeFile,freezeHashFile,readFreeze=readOos2sFreeze}={}) {
  ...
  try {
    ...
    if(history.some(r=>(r.protocolId===freeze.protocolId||r.campaignId===freeze.campaignId)&&!entries.some(e=>e.manifest.recordIds.includes(r.id)&&sameMembership(r,e.manifest)&&r.metadata?.context?.monitorCycleId===e.manifest.cycleId)))return failed('OOS2S_IDENTITY_MISMATCH');
