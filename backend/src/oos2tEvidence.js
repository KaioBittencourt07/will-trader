...
export const OOS2T_PROTOCOL='will-edge-gate-oos2t-v1';
...
export function verifyOos2tCycle(entry,history=[]) {
  ...
  if(m.protocolId!==OOS2T_PROTOCOL)reasons.push('PROTOCOL_INVALID');
  ...
  const integrityReasons=new Set(['MANIFEST_INVALID','PROJECTION_INVALID','PROTOCOL_INVALID','MANIFEST_INVALID_STATE','OBSERVATION_TERMINAL_INVALID','DUPLICATE_RECORD','MISSING_RECORD','EXTRA_RECORD','MEMBERSHIP_MISMATCH','FAILURE_HAS_RECORDS','CREATION_FIELDS_MUTATED']);
  const valid=!reasons.some(x=>integrityReasons.has(x));
  return {valid,complete:reasons.length===0,reasons:[...new Set(reasons)],records:rows,classes,observationOutcome:terminal?.outcome??null,observationFailureReason:terminal?.outcome==='OPERATIONAL_FAILURE'?terminal.reasonCode:null,
    tradeTerminalRecords:classes.filter(x=>x.class==='TRADE_TERMINAL').length,noTradeRecords:classes.filter(x=>x.class==='NO_TRADE_TERMINAL').length,dataInvalidRecords:classes.filter(x=>x.class==='DATA_INVALID_TERMINAL').length};
}
